...
def get_from_ghibli_api(entity_type: str) -> list:
    ...
    if response.status_code != 200:
        abort(Response(f'Failed to get {entity_type} from Ghibli API', status=502))
...
def list_films_and_people() -> Response:
    """
    Fetch films and people info via Ghibli API and create mapping of film names to people.
    No "schema validation" is performed on the JSON obtained from the Ghibli API,
    but a production-ready implementation should do this. It could be as simple as catching
    the KeyError exceptions a bad schema would trigger and returning a 502 status code.
    """
    ...
    # Inconveniently, the "films" list provided by the people-endpoint contains film URLs, not IDs. Must parse.
    FILM_URL_RX = re.compile(r'^.*/(?P<film_id>[0-9a-f\-]+)$')

    # Add people to their respective films, as given by their "films" list.
    # Skip those people whose film ID can not be parsed or is missing in the films list.
